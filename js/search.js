...
function search(query){
    switch(query.substr(0, 2)){
		...
		case "!t":
            query = query.substr(3);
            window.location = "https://torrentz2.eu/search?f=" + 
                query.replaceChars(" ", " ");
            break;
			
        case "!l":
            query = query.substr(3);
            window.location = "https://www.rl-trades.com/#h0=" + 
                query.replaceChars(" ", " ");
            break;
			
		case "!a":
            query = query.substr(3);
            window.location = "https://www.amazon.co.uk/s/ref=nb_sb_noss?url=search-alias%3Daps&field-keywords=" +
                query.replaceChars(" ", "+");
            break;

		case "!e":
            query = query.substr(3);
            window.location = "http://www.ebay.co.uk/sch/i.html?_from=R40&_sacat=0&_nkw=" +
                query.replaceChars(" ", "+");
            break;
		...
		case "!i":
            query = query.substr(3);
            window.location = "http://www.imdb.com/find?s=all&q=" +
                query.replaceChars(" ", "+");
            break;
			
		case "!u":
            query = query.substr(3);
            window.location = "https://www.urbandictionary.com/define.php?term=" +
                query.replaceChars(" ", "+");
            break;
        ...
        default:
            window.location="https://search.paulla.asso.fr/?q=" +
                query.replaceChars(" ", "+");
    }
}
